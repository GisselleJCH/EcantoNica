...
import 'package:barra/pages/lugares/dep1-madri.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const depomadri(title: ''),
    );
  }
}

class depomadri extends StatefulWidget {
  const depomadri({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  State<depomadri> createState() => _DeportesMadridState();
}

class _DeportesMadridState extends State<depomadri> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            child: ListView(
              children: <Widget>[
                buildContainer('assets/dmadri1.jpg', 'Estadio Santiago Bernabéu', () {
                  // Acción del botón "Ver más" para Estadio Santiago Bernabéu
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => depomadr1()),
                  );
                }),
              ],
            ),
