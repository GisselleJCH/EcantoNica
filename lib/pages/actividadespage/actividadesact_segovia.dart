...
import 'package:barra/pages/lugares/activi1-sego.dart';
import 'package:barra/pages/lugares/activi2-sego.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const actisego(title: ''),
    );
  }
}

class actisego extends StatefulWidget {
  const actisego({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  State<actisego> createState() => _MyHomePageState();
}

class _MyHomePageState extends State<actisego> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            child: ListView(
              children: <Widget>[
                buildContainer('assets/sego1.jpg', 'Reserva Natural Tizate', () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => sego1()),
                  );
                }),
                const SizedBox(height: 20),
                buildContainer('assets/sego2.jpg', 'Laguna de Jalapa', () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => sego2()),
                  );
                }),
              ],
            ),
