...
import 'package:barra/pages/lugares/dep1-mana.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const depomana(title: ''),
    );
  }
}

class depomana extends StatefulWidget {
  const depomana({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  State<depomana> createState() => _DeportesManaguaState();
}

class _DeportesManaguaState extends State<depomana> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            child: ListView(
              children: <Widget>[
                buildContainer('assets/dman1.jpg', 'Estadio Nacional Dennis Martínez', () {
                  // Acción del botón "Ver más" para Estadio Nacional Dennis Martínez
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => manade1()),
                  );
                }),
