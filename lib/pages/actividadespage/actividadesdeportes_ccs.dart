...
import 'package:barra/pages/lugares/dep1-css.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const depoccs(title: ''),
    );
  }
}

class depoccs extends StatefulWidget {
  const depoccs({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  State<depoccs> createState() => _DeportesCaboCruzState();
}

class _DeportesCaboCruzState extends State<depoccs> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            child: ListView(
              children: <Widget>[
                buildContainer('assets/dccs1.jpg', 'Estadio Municipal de Bluefields', () {
                  // Acción del botón "Ver más" para Estadio Municipal de Bluefields
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => deccs1()),
                  );
                }),
              ],
            ),
