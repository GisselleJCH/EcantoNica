...
import 'package:barra/pages/lugares/parq1-sego.dart';
import 'package:barra/pages/lugares/parq2-sego.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const Parsego(title: ''),
    );
  }
}

class Parsego extends StatefulWidget {
  const Parsego({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  State<Parsego> createState() => _ParsegoState();
}

class _ParsegoState extends State<Parsego> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            child: ListView(
              children: <Widget>[
                buildContainer('assets/par1sego.jpg', 'Parque Central de Ocotal', () {
                  // Acción del botón "Ver más" para Parque Central de Ocotal
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => parsego1()),
                  );
                }),
                const SizedBox(height: 20),
                buildContainer('assets/par2sego.jpg', 'Parque Rubén Darío', () {
                  // Acción del botón "Ver más" para Parque Rubén Darío
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => parsego2()),
                  );
                }),
              ],
            ),
