...
import 'package:barra/pages/lugares/parq1-css.dart';
import 'package:barra/pages/lugares/parq2-ccs.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const Parccs(title: ''),
    );
  }
}

class Parccs extends StatefulWidget {
  const Parccs({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  State<Parccs> createState() => _ParccsState();
}

class _ParccsState extends State<Parccs> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            child: ListView(
              children: <Widget>[
                buildContainer('assets/pccs1.jpg', 'Parque Central de Bluefields', () {
             
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => parqccs1()),
                  );
                }),
                 buildContainer('assets/pccs2.jpg', 'Parque El Carmen', () {
    
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => parqccs2()),
                  );
                }),
...
class DetailsScreen extends StatelessWidget {
  ...
  const DetailsScreen(this.placeName);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Detalles de $placeName'),
      ),
      body: Center(
        child: Text('Detalles de $placeName'),
      ),
    );
  }
}
