...
import 'package:barra/pages/lugares/dep1-esteli.dart';
import 'package:barra/pages/lugares/dep2-esteli.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const depesteli(title: ''),
    );
  }
}

class depesteli extends StatefulWidget {
  const depesteli({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  State<depesteli> createState() => _DeportesEsteliState();
}

class _DeportesEsteliState extends State<depesteli> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            child: ListView(
              children: <Widget>[
                buildContainer('assets/de1.jpg', 'Estadio Norberto Herrera', () {
                  // Acción del botón "Ver más" para Estadio Norberto Herrera
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => deeste1()),
                  );
                }),
                const SizedBox(height: 20),
                buildContainer('assets/de2.jpg', 'Complejo Deportivo Alejandro León', () {
                  // Acción del botón "Ver más" para Complejo Deportivo Alejandro León
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => deeste2 ()),
                  );
                }),
              ],
            ),
