...
import 'package:barra/pages/lugares/parq1-riosan.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const Parriosan(title: ''),
    );
  }
}

class Parriosan extends StatefulWidget {
  const Parriosan({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  State<Parriosan> createState() => _ParriosanState();
}

class _ParriosanState extends State<Parriosan> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            child: ListView(
              children: <Widget>[
                buildContainer('assets/parsan1.jpg', 'Parque Central de San Carlos', () {
                  // Acción del botón "Ver más" para Parque Central de San Carlos
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => parrio1()),
                  );
                }),
              ],
            ),
