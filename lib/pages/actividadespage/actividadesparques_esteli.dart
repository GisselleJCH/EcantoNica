...
import 'package:barra/pages/lugares/parq1-esteli.dart';
import 'package:barra/pages/lugares/parq2-esteli.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const ParqEsteli(title: ''),
    );
  }
}

class ParqEsteli extends StatefulWidget {
  const ParqEsteli({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  State<ParqEsteli> createState() => _ParqEsteliState();
}

class _ParqEsteliState extends State<ParqEsteli> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            child: ListView(
              children: <Widget>[
                buildContainer('assets/par1.jpg', 'Parque Central de Estelí', () {
                  // Acción del botón "Ver más" para Parque Central de Estelí
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => parest1()),
                  );
                }),
                const SizedBox(height: 20),
                buildContainer('assets/par2.jpg', 'Parque de la Independencia', () {
                  // Acción del botón "Ver más" para Parque de la Independencia
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => parest2()),
                  );
                }),
              ],
            ),
