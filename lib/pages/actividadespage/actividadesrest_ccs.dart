...
import 'package:barra/pages/lugares/resta1-ccs.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const Restccs(title: ''),
    );
  }
}

class Restccs extends StatefulWidget {
  const Restccs({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  State<Restccs> createState() => _RestccsState();
}

class _RestccsState extends State<Restccs> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            child: ListView(
              children: <Widget>[
                buildContainer('assets/rccs1.jpg', 'Restaurante El Castillo', () {
                  // Acción del botón "Ver más" para Restaurante El Castillo
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => restaccs1()),
                  );
                }),
