...
import 'package:barra/pages/lugares/activi1-css.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const actccs(title: ''),
    );
  }
}

class actccs extends StatefulWidget {
  const actccs({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  State<actccs> createState() => _MyHomePageState();
}

class _MyHomePageState extends State<actccs> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          const SizedBox(height: 20), // Separación entre el título y los contenedores
          Expanded(
            child: ListView(
              children: <Widget>[
                buildContainer('assets/actccs1.jpg', 'Reserva de Biósfera Indio Maíz', () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => const actccs1()),
                  );
                }),
