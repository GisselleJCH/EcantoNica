...
import 'package:barra/pages/lugares/sende1-mana.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const Sendemana(title: ''),
    );
  }
}

class Sendemana extends StatefulWidget {
  const Sendemana({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  State<Sendemana> createState() => _SendemanaState();
}

class _SendemanaState extends State<Sendemana> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            child: ListView(
              children: <Widget>[
                buildContainer('assets/sman1.jpg', 'Cascada de Chiltepe: Un tesoro natural en Managua', () {
                  // Acción del botón "Ver más" para Cascada de Chiltepe
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => sendemana1()),
                  );
                }),
