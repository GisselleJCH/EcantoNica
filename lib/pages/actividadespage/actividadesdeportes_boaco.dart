...
import 'package:barra/pages/lugares/dep1-boaco.dart';
import 'package:barra/pages/lugares/dep2-boaco.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const depoboaco(title: ''),
    );
  }
}

class depoboaco extends StatefulWidget {
  const depoboaco({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  State<depoboaco> createState() => _DeportesBoacoState();
}

class _DeportesBoacoState extends State<depoboaco> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: <Widget>[
          ...
          Expanded(
            child: ListView(
              children: <Widget>[
                buildContainer('assets/d1.jpeg', 'Estadio Municipal de Boaco', () {
                  // Acción del botón "Ver más" para Estadio Municipal de Boaco
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => deboa1()),
                  );
                }),
                const SizedBox(height: 20),
                buildContainer('assets/d2.jpg', 'Complejo Deportivo Alexis Argüello', () {
                  // Acción del botón "Ver más" para Complejo Deportivo Alexis Argüello
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => deboa2()),
                  );
                }),
              ],
            ),
